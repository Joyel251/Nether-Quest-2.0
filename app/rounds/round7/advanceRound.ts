...
export async function advanceRound() {
    try {
        ...
        await supabase.auth.updateUser({ data: { round: 8 }  });
...
export async function canAdvance(): Promise<{ advanced: boolean; eliminated: boolean }> {
    try {
        ...
        const count = await prisma.round7Submission.count();

        const permissibleRaw = user.user_metadata?.participant_limits?.["7"];
